...
import { normalizeUrl } from '../../../_lib/auth.js'
...
export default async function handler(req, res) {
  ...
  const backend_url = normalizeUrl(req.body.backend_url)
  ...
  const { error } = await supabase
    .from('backends')
    .insert({ url: backend_url, owner_id: userData.id, claimed_at: new Date().toISOString()})
